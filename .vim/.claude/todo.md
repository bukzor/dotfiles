...
- [ ] Replace lsp-inlayhints.nvim with built-in `vim.lsp.inlay_hint`
  - [ ] In `M.on_attach_lspconfig`, swap `require("lsp-inlayhints").on_attach(client, bufnr)` for `vim.lsp.inlay_hint.enable(true, { bufnr = bufnr })`
  - [ ] Remove `lvimuser/lsp-inlayhints.nvim` from plugins.lua
  - [ ] Remove the `require("lsp-inlayhints").reset()` call in `M.unload`
