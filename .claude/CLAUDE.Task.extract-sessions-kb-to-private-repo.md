...
pollutes that repo's log/blame. Solution: a private GitHub repo,
wired back into dotfiles as a **git submodule** at the same path
(user's explicit preference, 2026-07-09 — they already lean on
submodules elsewhere, e.g. `repo/scratch`). The churn concern is
mitigated by only bumping the dotfiles gitlink pointer as needed, not
after every sessions.kb-side commit.

Steps:

- [x] Confirm with user: repo name, history preservation, per-host subdir layout, submodule vs plain clone
- [ ] Create `bukzor/bukzor.claude-sessions` private repo via `gh repo create`
...
`~/.claude/sessions.kb/extract-sessions-kb-to-private-repo.md`)

1. **Repo name**: `bukzor/bukzor.claude-sessions` (private) — `bukzor.`
   prefix survives forks.
...
```bash
gh repo create bukzor/bukzor.claude-sessions --private --description "claude-code session index"
```
...
sessions.kb: extract to private bukzor.claude-sessions repo, wire back
in as a submodule
...
- **Secrets in history.** `filter-repo` preserves everything; run
  ...
  Not currently on PATH on this host (`penguin`) — `brew install
  gitleaks` before step 3.
