...
- [ ] [reunify-dotfiles.kb/2026-07-06-003-Main-only-path-triage.md](reunify-dotfiles.kb/2026-07-06-003-Main-only-path-triage.md) — all keep/delete decisions made and executed 2026-07-12 (bias flipped to keep-by-default; 3 tooling-breaking paths deleted, `bukzor.readline` ported+wired into svelte's `lib/pythonpath/bukzor/`); only two minor formal-hygiene checklist items left open (permanent dangling-ref check script, final path-list regen) — task-complete in substance, left unchecked pending user call on whether those still matter
