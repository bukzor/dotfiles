...
# Destructure must-read.d/before/creating-documentation.md into references.kb/ llm.d structure
...
- Single large file in must-read.d/before/creating-documentation.md
- Some content missing (ADRs, ROADMAP, docs/milestones, docs/architecture)
- Some content incorrect (will be fixed in prior task)

## Proposed Solution

Split into references.kb/ with llm.d-style categorization:
...
2. [~] Split content into small focused files (using successive subtraction)
   ...
   - [x] Identify and extract next category from remainder
     - [x] guidelines.kb/ (linking, DRY, subdirs, entry-points, anti-patterns)
     - [x] workflows.kb/ (LLM consumption, maintenance, checklist, when to deviate)
...
- Should we keep creating-documentation.md as a high-level overview that links to references.kb/?

## Success Criteria

- [ ] All content from creating-documentation.md is in references.kb/
...
Depends on: "Fix errors in must-read.d/before/creating-documentation.md" task completing first
