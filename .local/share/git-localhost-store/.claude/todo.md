...
- [ ] Fold submodule `core.worktree` absolute-path rewriting into the migration tool. Manual logic from devlog 2026-05-08-000 not yet codified; none of the recently-batched candidates needed it but the next submodule-bearing legacy repo will.
- [ ] Stage-3 hard cases (devlog 2026-05-08-001):
  - `~/repo/github.com/bukzor/bukzor-agent-skills` — FETCH_HEAD conflict (wt vs store) + unknown `filter-repo/` directory
  - `~/claude/amazon-searches` — gitfile still points at the pre-rename `claude-empty` store (per commit 1210b52)
