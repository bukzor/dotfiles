...
- [ ] Extend `bin/audit-gitfiles` to also report stores whose decoded workdir-path exists but has no `.git` — caught by the 2026-05-11 wider audit (`claude-plugins-official` case). Today's `audit-gitfiles` scans workdir-side gitfiles only, so a workdir whose `.git` is gone is invisible. Augmentation: walk `<repos>/`, decode each name, check whether the decoded path exists and lacks a `.git`. Reference script: `trash/2026-05-11-migration/audit-classify.sh` (gitignored).
