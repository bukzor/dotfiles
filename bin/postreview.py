...
#
REVIEWBOARD_URL = "https://reviewboard.yelpcorp.com"
...
ADD_REPOSITORY_DOCS_URL = \
    'http://www.reviewboard.org/docs/manual/dev/admin/configuration/repositories/#managing-repositories'
...
class ReviewBoardServer(object):
    ...
    def __init__(self, url, info, cookie_file):
        ...
        opener.addheaders = [('User-agent', 'RBTools/custom-in-nail-sys-bin')]
    ...
    def http_post(self, path, fields, files=None):
        ...
        # Patch by buck. See
        # https://jira.yelpcorp.com/browse/REVBOARD-14 for details
...
class GitClient(SCMClient):
    ...
    def get_repository_info(self):
        ...
        if options.repository_url:
            ...
            # During gitolite migration, chop off git@ to make rb happy
            if url.find('sysgit') == -1 and url.startswith("git@"):
                 url = url[4:]
    ...
    def make_diff(self, ancestor, commit=""):
        ...
        if self.type == "svn":
            diff_lines = execute([self.git, "diff", "--no-color", "--no-prefix",
                                  "--no-ext-diff", "--ignore-submodules", "-r", "-u", rev_range],
                                 split_lines=True)
...
def parse_options(args):
    parser = OptionParser(usage="%prog [-pond] [-r review_id] [changenum]",
                          version="RBTools custom-in-nail-sys-bin")
...
def main():
    ...
    debug('RBTools custom-in-nail-sys-bin')
