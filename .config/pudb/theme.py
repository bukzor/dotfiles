...
SyntaxGroup = str
SyntaxColor = t.Union[
    t.Tuple[str], t.Tuple[str, str]  # foreground  # foreground, background
]
SyntaxEntry = t.Union[SyntaxGroup, SyntaxColor]  # link
...
all_syntax_items = sorted(
    ...
    | set(palette)
    | set(
        value for value in palette.values() if isinstance(value, SyntaxGroup)
    )
)
...
if DEBUG:
    f = (Path(__file__).parent / "palette.py").open("w")
...
debug(
    f"palette.update({json.dumps(palette, sort_keys=True, indent=2)})\n"
)
