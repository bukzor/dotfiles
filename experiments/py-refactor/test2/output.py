...
class search(StdCmd, RequestMapper, BizSiteMapLinks):
	def rules(self):
		return (
			(And(GET(), Path(r'^search/?$')), self.search),
			(And(GET(), Path(r'^concierge/?$')), self.concierge),
			(And(GET(), Path(r'^snippet/?$')), self.snippet),
		)
